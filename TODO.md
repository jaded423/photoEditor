...
- [ ] **Rebuild + redistribute PhotoEditor.app with the focal cut** (added 2026-09-04). `./build_app.sh`, zip `dist/PhotoEditor.app`, upload to the Elevated Drive copy ("Manage versions" → v3), send the photographer the one-liner `xattr -cr PhotoEditor.app`. First run on her Mac downloads ~375MB of SAM weights next to BiRefNet.
      ...
      resume: code + docs landed and `dist/PhotoEditor.app` BUILT 2026-09-04 (359MB, verified under the pinned build venv: rembg 2.0.67 / numpy 2.2.6 / no opencv-contrib). Left: zip → Elevated Drive v3 → tell the photographer. Memory note: BiRefNet already peaked ~9.5GB/photo on the old path, new path 4–8GB — no regression, but don't run two batches at once.
